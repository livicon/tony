<script>
  ...
  const openSelectdep = () => (showdep = !showdep);
...
<div class="w-full col-span-2 grid gap-2 grid-cols-10">
  <div class={(!showdep)?" col-span-9 w-46 h-12 ":"hidden"} >
    <div class=" grid grid-cols-3 gap-1 mt-2 mx-auto w-44">
  ...
  <div class={(showdep)?"flex items-center justify-center bg-white rounded-xl col-span-6":"hidden"} >
    <input
      type="text"
      placeholder="مبدا"
      class="p-3 bg-transparent text-sm w-32"
    />
    <button class="flex justify-center items-center border rounded-full">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="h-6 w-6"
        fill="none"
  ...
  <div on:click={() => toggleDate()} class={(showdep)?"w-46 h-12 bg-white rounded-xl flex items-center justify-center col-span-2":"hidden"} >
    {datas ? datas : "تاریخ"}
  </div>
  <button class={(showdep)?"w-12 h-12 bg-blue rounded-xl text-white flex items-center justify-center mr-auto":"hidden"}>
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class="h-6 w-6 "
      fill="none"
  ...
  <button
    on:click={openSelectdep}
    class="w-12 h-12 bg-white rounded-xl text-gray-600 flex items-center justify-center mr-auto"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class={showdep == true
...
<div
  ...
  aria-hidden="true"
  class={show == true
    ? " w-4/5 h-full overflow-y-auto overflow-x-hidden m-auto absolute right-0 left-0 top-80 z-50 justify-center items-center h-modal md:h-full md:inset-0"
    : "hidden"}
>
